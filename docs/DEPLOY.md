...
- Открытый порт **90** (NGINX) или туннель/прокси до него.
...
Compose-файл задаёт `name: skin`, поэтому контейнеры будут называться `skin-backend-1`, `skin-nginx-1` и т.д. Внутренние имена сервисов (`postgres`, `redis`, `minio`, `ml`, `backend`, `frontend`) остаются короткими. Сеть `skin-ai-network` используется как внешняя общая сеть для интеграции с соседними сервисами; если её ещё нет, создайте её командой `docker network create skin-ai-network` перед первым запуском.
...
По умолчанию в текущем `docker-compose.yml` консоль MinIO наружу не публикуется. Если нужен доступ к web console, добавьте отдельный `ports`-маппинг для `9001` вручную.
...
| `JWT_EXPIRE_MINUTES` | `240` (4 ч) | Время жизни обычного токена |
| `JWT_REMEMBER_ME_EXPIRE_MINUTES` | `240` (4 ч) | Время жизни токена в режиме “запомнить меня” |
...
Если вы хотите, чтобы работали регистрация, подтверждение email и сброс пароля, SMTP-настройки обязательны. Без `SMTP_HOST` и `SMTP_USER` backend не сможет отправлять письма, а соответствующие пользовательские сценарии будут завершаться ошибкой.
...
Внутренние порты (не проброшены наружу по умолчанию): PostgreSQL `:5432`, backend `:9000`, frontend `:3000`, ml `:8000`, redis `:6379`, minio S3 `:9000`, MinIO console `:9001`.
...
3. Убедитесь, что `FRONTEND_PUBLIC_URL` указывает на адрес, доступный из браузера пользователя (не `localhost`, если деплой удалённый или используется туннель).
