...
def load_model(path_to_model: str)-> nn.Module:
    """
    Loads a pre-trained model for pattern classification.
    Args:
        path_to_model (str): Path to the saved model weights.
    Returns:
        nn.Module: Loaded model set to evaluation mode.
    """
...
def predict(image_np: np.ndarray) -> str:
    """
        Predicts the dominant pattern in an input image using a pre-trained model.
        Args:
            image_np (np.ndarray): Input image as a NumPy array.
        Returns:
            str: Predicted class label as a string.
    """
