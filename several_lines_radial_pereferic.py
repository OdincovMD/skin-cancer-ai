...
# переименовано с several_lines_radial_edges_pereferic.joblib -> several_lines_radial_pereferic.joblib
MODEL_PATH = os.path.join('weight', 'several_lines_radial_pereferic.joblib')
...
def main(img: np.ndarray, mask) -> str:
    ...
    Returns:
        str: "Светлая бесструктурная область" or "Тёмная бесструктурная область".
    """
    model = get_model()
    features = get_image_features(img, mask)
    df = pd.DataFrame([features])
    pred = model.predict(df)
    return 'Светлая бесструктурная область' if pred == 1 else 'Тёмная бесструктурная область'
