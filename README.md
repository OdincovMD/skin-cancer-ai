...
5. Frontend опрашивает **`GET /classification-jobs/{job_id}`**: сначала показывает стадии обработки, затем итоговую классификацию, интерактивное дерево решений с управлением масштабом и, при полном анализе, дозагружает `description_status`, `description`, `important_labels` и другие поля description pipeline.
...
- **Главная / Workspace** — загрузка с лупой, выбор режима анализа, опрос задания, интерактивное дерево решений с масштабированием и сбросом вида.
- **Профиль** — личные данные, безопасность, доступ по API, история классификаций, превью из хранилища и восстановление доступа к аккаунту.
